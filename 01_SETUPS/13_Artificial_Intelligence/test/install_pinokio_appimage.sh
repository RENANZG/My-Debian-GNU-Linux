...
# USAGE:
#  Make it executable:
#   $ sudo chmod +x ~/Downloads/Pinokio_AppImage.sh
#  Then run
#   $ sudo bash ~/Downloads/Pinokio_AppImage.sh
########################################################################

APP=pinokio
SITE="pinokiocomputer/pinokio"
...
version=$(curl -Ls https://api.github.com/repos/pinokiocomputer/pinokio/releases | jq '.' | grep browser_download_url | grep -w -v arm64  | grep -i appimage | cut -d '"' -f 4 | head -1)
wget $version
