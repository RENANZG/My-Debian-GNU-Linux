...
# USAGE:
#  Make it executable:
#   $ sudo chmod +x ~/Downloads/Embedded-ide_AppImage.sh
#  Then run
#   $ sudo bash ~/Downloads/Embedded-ide_AppImage.sh
########################################################################
...
set -u
APP=embedded-ide
SITE="martinribelotta/embedded-ide"
...
version=$(curl -Ls https://api.github.com/repos/martinribelotta/embedded-ide/releases | sed 's/[()",{} ]/\n/g' | grep -oi "https.*mage$" | grep -vi "i386|i686|aarch64|arm64|armv7l" | head -1)
wget "$version" || exit 1
