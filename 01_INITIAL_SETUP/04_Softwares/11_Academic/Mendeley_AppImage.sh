...
# DESCRIPTION: 
# Make it executable:
# $ sudo chmod +x ~/Downloads/Mendeley_AppImage.sh
# Then run
# $ sudo bash ~/Downloads/Mendeley_AppImage.sh
########################################################################
...
version=$(curl -Ls https://www.mendeley.com/download-reference-manager/linux | grep -Eo "(http|https)://[a-zA-Z0-9./?=_%:-]*" | sort -u | grep -i appimage)
wget $version
