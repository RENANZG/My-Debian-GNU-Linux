...
########################################################################
# File Name    : git_setup_ssh_file.sh
# Description  : This script sets up Git with SSH keys for 
#                authentication and configures your Git user information. 
#                It generates an `ed25519` SSH key if one does not 
#                already exist, and writes the public key to a temporary 
#                file. You can manually copy this key to your GitHub 
#                account.
# Dependencies : git; ssh-keygen
# Usage        : • Make the script executable with:
#                 chmod +x git_setup_ssh_file.sh
#                • Run the script with:
#                 bash git_setup_ssh_file.sh
...
# Optionally, open the file with the default text editor (can be replaced with `nano`, `vim`, etc.)
xdg-open "$PUB_KEY_TEMP_FILE" &> /dev/null
