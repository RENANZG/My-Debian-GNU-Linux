...
sdX                   0:0    0    100G  0 disk
├─sdX1                0:0    0    100G  0 part
sdY                   0:0    1    100G  0 disk
└─sdY1                0:0    1    100G  0 disk
</pre>

<p>1.4 Boot a Debian Live CD.</p>

<p>1.5 Connect to internet and open terminal. Live CD has sudo enabled by default, but this is the commands to acess sudo privileges</p>
...
<p>In it b|B is bytes, m|M is MiB and g|G is GiB.</br>
...
<p><em>*Press CTRL+C to back only one instance</em></p>

<!-- #################### -->

2.4 Create partitions:</br>

<h4>Use GPT to boot the disk in UEFI mode. Enter <code>g</code> to create a new empty GPT partition table:</h4>
...
<p>Keep in mind the encrypted volume will be /dev/sdX2 on EFI systems, since /dev/sdX1 is taken up by the EFI partition.</p>
...
<p>Run the <code>n</code> command to create a new partition:</p>

<pre>
Command (m for help): n
</pre>

<p>UEFI systems typically need an EFI system partition, BIOS systems that are partitioned with GPT require a BIOS boot partition if GRUB is used as the bootloader. You’ll be prompted to enter the partition number, hit <code>“Enter”</code> to use the default value (1), EFI system:</p>

<pre>
Partition number (1-128, default 1):
</pre>

<p>Next, the command will ask you to specify the first sector. Generally it is always recommended to use the default values for the first value. Hit <code>“Enter”</code> to use the default value (2048):</p>

<pre>
First sector (2048-100000000, default 2048):
</pre>

<p>On the next prompt, you’ll need to enter the last sector. You can use an absolute value for the last sector or relative value to the start sector, using the <code>+</code> symbol following by the partition size. The size can be specified in kibibytes (<code>K</code>), mebibytes (<code>M</code>), gibibytes (<code>G</code>), tebibytes (<code>T</code>), or pebibytes (<code>P</code>).</p>

<p>Press <code>l</code> to get a list of partition types and then press <code>t</code> to change the type.</p>

<pre>
Last sector, +/-sectors or +/-size{K,M,G,T,P} (2048-500118158, default 500118158):
</pre>

<p>Enter <code>+100M</code> to set the partition size to 100 MiB:</p>
...
</p>Once done creating partitions, use the <code>p</code> command to display the new partition table and <code>w</code> to write.</p>

<!-- ################################# -->

2.6 Configure LUKS partitions. If not, install cryptsetup utility and recheck your disk targets. </br>

</p>You can use the rest of the disk to create an LVM with root, home and swap. Alternatively you can leave e MiB.
</pre>

</p>Once done creating partitions, use the <code>p</code> command to displaympty space at the end of the disk to install another system or for a separate partition.</p>
...
2.7 Create a partition</br>

<p>First we need to allocate the space and then encrypt it.</p>
...
<p>You’ll be prompted to enter the partition number, hit <code>“Enter”</code> to use the XXXXXXXXXXXX, XXXXXXXXXXX:</p>
...
<pre>
Partition number (1-128, default 1): XX
</pre>
...
<p>On the next prompt, you’ll need to enter the last sector. Enter <code>+34G</code> to set the partition size to 34 GiB:</p>

<pre>
Last sector, +/-sectors or +/-size{K,M,G,T,P} (2048-500118158, default 500118158): +34G
</pre>

<pre>
(...)Created a new partition 19 of type 'Linux filesystem' and of size 34 GiB.
</pre>

<p>Once done creating partitions, use the <code>p</code> command to display the new partition table and w to write.</p>

<!-- ################################# -->

2.8 Create a partition with the LUKS label.</br>

<p>Keep in mind the encrypted volume will be /dev/sdX2 on EFI systems, since /dev/sdX1 is taken up by the EFI partition. In the example we will leave sdX3 empty.</p>

<p>In this example, we will propose a size for LUKS. Feel free to use the entire disk.</p>
...
<p>Choose a memorable, long and high entropy password. 70 bit of entropy is a good choice.</p>
