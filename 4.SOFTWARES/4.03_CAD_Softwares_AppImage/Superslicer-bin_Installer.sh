...
# USAGE          :
# Make it executable:
# $ sudo chmod +x ~/Downloads/Superslicer-bin_Installer.sh
# Then run
# $ sudo bash ~/Downloads/Superslicer-bin_Installer.sh
########################################################################
...
version=$(wget -q https://api.github.com/repos/$REPO/releases/latest -O - | grep -w -v i386 | grep -w -v i686 | grep -w -v aarch64 | grep -w -v arm64 | grep -w -v armv7l | grep browser_download_url | grep -i appimage | cut -d '"' -f 4 | head -1)
wget $version
