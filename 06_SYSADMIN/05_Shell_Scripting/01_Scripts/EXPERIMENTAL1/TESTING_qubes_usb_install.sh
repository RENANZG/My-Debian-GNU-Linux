...
download_qubes() {
    ...
    QUBES_VERSION=$(curl -s -L https://www.qubes-os.org/downloads/ | grep -oP 'Qubes-R\K[0-9.]+-x86_64' | head -1)
    ...
    QUBES_ISO_URL="https://mirrors.edge.kernel.org/qubes/iso/Qubes-R$QUBES_VERSION.iso"
    QUBES_SIG_URL="https://mirrors.edge.kernel.org/qubes/iso/Qubes-R$QUBES_VERSION.iso.asc"
    QUBES_KEY_URL="https://keys.qubes-os.org/keys/qubes-release-4-signing-key.asc"
    ...
    curl -L -o qubes_data/qubes-release-4-signing-key.asc $QUBES_KEY_URL
}

# Function to verify Qubes ISO
verify_qubes() {
    echo "[+] Verifying Qubes ISO..."
    gpg --no-default-keyring --keyring ./qubes_data/tmp_keyring.pgp --import ./qubes_data/qubes-release-4-signing-key.asc

    if gpg --no-default-keyring --keyring ./qubes_data/tmp_keyring.pgp --fingerprint 427F 11FD | grep "427F 11FD 0FAA 4B08 15C2  67FD 5C65 68D3 8D17 93A9"; then
    ...
    if gpg --no-default-keyring --keyring ./qubes_data/tmp_keyring.pgp --verify ./qubes_data/Qubes-R$QUBES_VERSION.iso.asc ./qubes_data/Qubes-R$QUBES_VERSION.iso; then
