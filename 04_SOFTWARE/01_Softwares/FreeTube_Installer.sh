...
########################################################################
# VISIT: https://github.com/FreeTubeApp/FreeTube
...
version=$(wget -q https://api.github.com/repos/$REPO/releases -O - | grep amd64 | grep -w -v i686 | grep -w -v aarch64 | grep -w -v arm64 | grep -w -v armv7l | grep browser_download_url | grep -i appimage | cut -d '"' -f 4 | head -1)
wget $version
