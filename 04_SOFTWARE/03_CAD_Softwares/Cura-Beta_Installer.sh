...
# USAGE          :
# Make it executable:
# $ sudo chmod +x ~/Downloads/Cura-Beta_Installer.sh
# Then run
# $ sudo bash ~/Downloads/Cura-Beta_Installer.sh
########################################################################
...
version=$(wget -q https://api.github.com/repos/$REPO/releases -O - | grep -w -v i386 | grep -w -v i686 | grep -w -v aarch64 | grep -w -v arm64 | grep -w -v armv7l | grep browser_download_url | grep -i beta  | grep -i appimage | cut -d '"' -f 4 | head -1)
wget $version
