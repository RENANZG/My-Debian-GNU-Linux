...
1.6 Find your devices and figure out which device you will be using system on. Make sure you will use the correct name for your label. You could use these commands:
...
*Take care with dependencies.</br>

<h3>2. Manual Partitioning</h3>

2.1 You need to determine minimum requirements for your system.</br>

2.2 EFI System Partition (ESP): create 100MB like Windows, you won't fill it up. Officially, It's recommended 300M.</br>

2.3 Boot: Ubuntu's default size is 466MiB (500MB). Note that boot partition is not always necessary. A separate /boot partition is only required if your boot loader is not capable of accessing the /boot directory that resides in /. For example, if the boot loader does not support that file system or if your / is on a stacked block device (e.g. software RAID, a encrypted volume or a LVM volume) and the boot loader does not have drivers for it.</br>

2.4 Root: Ubuntu recommends at least 5GB, but in practice that is far too small. At least 20GB makes a safer size.</br>

2.5 Swap: In theory, if you use hibernation, you could use up to twice your RAM. If you wish to use hibernation, and you might be working with huge files and systems (e.g. virtual machines, video editing and more all at the same time), be safe and choose twice your RAM. To use hibernation (a.k.a suspend to disk) it is advised to create the swap partition at the size of RAM. If you intend to never use hibernation or hybrid suspend, you can use a swap file (https://wiki.debian.org/Swap).</br>

2.6 Home: This is entirely a personal choice.</br>

2.7 Partitioning scenarios</br>
...
• For Root partition</br>
For 10GB EXT4 Drive</br>
It is displayed as 100.00GiB</br>

• For Home partition</br>

<b>In LVM2</b></br>
∙ Volume group:</br>
In it b|B is bytes, m|M is MiB and g|G is GiB.</br>
When the user want to round by 1000 - he can always go with 'byte' units i.e. -l 1000000b</br>
∙ Subvolume, logical volume:</br>
...
2.9 Login to the root live environment command line and find your disk targets.</br>
...
2.3 Creating Partition Table:</br>
...
Use GPT to boot the disk in UEFI mode. Enter g to create a new empty GPT partition table:</br>
...
2.7 Create a 16G partition with the home label, type 41.</br>
...
2.8 Create a 100M partition with the swap label, type 19.</br>
...
2.9 Configure LUKS partitions. Install cryptsetup utility and recheck your disk targets. </br>
...
*Use the same password for each encrypted partition (root, home and swap) to easy login in your system. Consult EFF Foundation site to know how to create a strong password. </br>
...
Format LUKS partition. Create a filesystem i.e. format filesystem, enter:</br>
